...
from datasets import TwoStreamBatchSampler
...
def main(args):
    ...
    if data_cfg['name'] == 'edge':
        dataset_real = EdgeDataset(
            data_root=data_cfg.img_folder_real,
            image_size=model_cfg.image_size,
            augment_horizontal_flip=data_cfg.augment_horizontal_flip,
            cfg=data_cfg
        )

        dataset_pseudo = EdgeDataset(
            data_root=data_cfg.img_folder_pseudo,
            image_size=model_cfg.image_size,
            augment_horizontal_flip=data_cfg.augment_horizontal_flip,
            cfg=data_cfg
        )

        dataset = ConcatDataset([dataset_real, dataset_pseudo])

    else:
        raise NotImplementedError

    num_real = len(dataset_real)
    num_pseudo = len(dataset_pseudo)
    primary_indices = list(range(num_real))
    secondary_indices = list(range(num_real, num_real + num_pseudo))

    secondary_batch_size = data_cfg.get('pseudo_batch_size', 4)
    ...
    if train_cfg.test_before: #在正式训练前，先用初始化的（未经训练的）模型对一个 Batch 的数据进行一次生成采样，确保数据流是通畅的，模型不会因为维度不匹配而在训练中途崩溃
...
class Trainer(object):
    def __init__(
        ...
        self.dl = cycle(dl)  # 说明训练不是按 Epoch 算的，而是按 step 算的
        ...
        # 学习率会随着 step 的增加而按照 0.96 的幂次缓慢衰减
